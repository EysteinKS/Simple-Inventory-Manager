...
const ReportDatePicker = () => {
  ...
  const lastChanged = useSelector((state: RootState) => state.auth.location.lastChanged.global)
  ...
  React.useEffect(() => {
    ...
    let dateString = `${lCDate.getFullYear()}-${lCDate.getMonth() + 1}-${LCDay()}T01:00:00`
    ...
    if(!isReloadingDates && sanitizedDate.getTime() !== lastDate.getTime() && dates.isLoaded){
  ...
  return (
    <PickerWrapper>
      ...
      />
      <button disabled={date === null} onClick={onLoadReport}>Last inn</button>
    </PickerWrapper>
  )
}
