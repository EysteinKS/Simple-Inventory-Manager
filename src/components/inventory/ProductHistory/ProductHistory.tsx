...
const ProductHistory: React.FC<IProps> = ({ isOpen, close }) => {
  const product = useSelector((state: RootState) => state.products.currentProduct) as IProduct
  ...
  const ordersByProduct = useSelector((state: RootState) => {
    ...
    let ordersWithProduct = allOrders.filter(order => {
      let orderedProductsByID = order.ordered.map(orderedProduct => orderedProduct.productID)
      return orderedProductsByID.includes(product.productID)
    })
    return ordersWithProduct
  })
  const memoizedOrders = React.useMemo(() => ordersByProduct.map(order => {
    return {
      type: "order",
      id: order.orderID,
  ...
  const salesByProduct = useSelector((state: RootState) => {
    ...
    let salesWithProduct = allSales.filter(sale => {
      let orderedProductsByID = sale.ordered.map(orderedProduct => orderedProduct.productID)
      return orderedProductsByID.includes(product.productID)
    })
    return salesWithProduct
  })
  const memoizedSales = React.useMemo(() => salesByProduct.map(sale => {
    return {
      type: "sale",
      id: sale.saleID,
  ...
  const loansByProduct = useSelector((state: RootState) => {
    ...
    let loansWithProduct = allLoans.filter(loan => {
      let orderedProductsByID = loan.ordered.map(orderedProduct => orderedProduct.productID)
      return orderedProductsByID.includes(product.productID)
    })
    return loansWithProduct
  })
  const memoizedLoans = React.useMemo(() => loansByProduct.map(loan => {
    return {
      type: "loan",
      id: loan.loanID,
...
const HistoryItem: React.FC<IHistoryItem> = ({ item }) => {
  ...
  const dateOpts = {day: "2-digit", month: "2-digit", year: "2-digit", hour: "2-digit", minute: "2-digit"}
