...
export const findReport = async (client: string, date: IDate) => {
  try {
    ...
    let reports = await secondaryFirestore.doc(`${client}/Reports`).get().then(doc => 
      doc.data()) as { byDate: IbyDate }
...
const isValidDate = (date: IDate) => ("year" in date && "month" in date && "day" in date)
