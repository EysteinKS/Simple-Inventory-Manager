...
export default (state: LoansState = initialState, {type, payload}: AnyAction) =>
  produce(state, draft => {
    switch(type){
      ...
      case action.EDIT_LOAN:
        ...
        draft.currentLoan = state.loans.find(loan => loan.loanID === payload) as ILoan
        break
      ...
      case action.SEND_LOAN:
        ...
        let sent: ILoan | any = state.loans.find(loan => (loan.loanID === payload))
      ...
      case action.DELETE_LOAN:
        ...
        let loanToDelete = state.loans.find(loan => loan.loanID === payload) as ILoan
        loanToDelete.isDeleted = true
        let deleteHistoryIndex: number = 0
        for(let i = 0; i < draft.history.length; i++){
          if(draft.loans[i].loanID > loanToDelete.loanID || i === draft.history.length - 1){
