...
import { getSectionFromFirestore, convertTimestampsToDates, setSectionToFirestore } from "../middleware/thunks";
...
export const loadLoans = () =>
  getSectionFromFirestore(thisSection,
    ...
    loadLoansFailure,
    (data) => {
      let loans = convertTimestampsToDates(data.loans, ["dateOrdered", "dateSent"])
      let history = convertTimestampsToDates(data.history, ["dateOrdered", "dateSent", "dateReceived"])
      return {
        loans,
        history,
...
export const saveLoans = (date: Date) =>
  setSectionToFirestore(
    ...
    (state) => {
      ...
      let loans = convertTimestampsToDates(l.loans, ["dateOrdered", "dateSent"])
      let history = convertTimestampsToDates(l.history, ["dateOrdered", "dateSent", "dateReceived"])
      return {
        loans,
        history,
...
export const didSendLoan = (id: number, ordered: IOrderedProduct[]): IThunkAction =>
  async (dispatch) => {
...
export const didReceiveLoan = (id: number, ordered: IOrderedProduct[]): IThunkAction =>
  async (dispatch) => {
    ordered.forEach(product => {
